...
| Handwritten note titles | Use Apple's recognized title when available; otherwise retain the stable fallback title | Corrected on selective branch | A newly recognized or corrected title should trigger only that note's export | Synthetic database + incremental fingerprint tests | Validate with a real handwritten note before release |
...
| Photo galleries | Export every gallery child plus a separate inline Obsidian image embed for each child | Corrected on selective branch | Child additions, removals, replacements, and ordering need runtime validation | Synthetic fallback extraction + multi-image export/embed tests | Validate a real multi-image gallery before release |
