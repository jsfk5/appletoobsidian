...
| Handwritten note titles | Use Apple's recognized title when available; otherwise retain the stable fallback title | Corrected and real-note validated on selective branch | A newly recognized or corrected title should trigger only that note's export | Synthetic database + incremental fingerprint tests + real handwritten note | Retest the final candidate before release |
| Bulleted lists | Markdown list syntax | Partial | Content changes should trigger export | Needs test | Add nested bullet fixture |
| Numbered lists | Markdown ordered list syntax with source-order and nested counters | Corrected on selective branch | Content changes should trigger export | Focused source-order, nesting, reset, formatting, type-boundary, and renderer-migration tests | Complete temporary-output renderer refresh validation before release |
...
| Multi-page scanned documents stored as internal gallery containers | Export every page image plus a separate inline Obsidian image embed for each page | Corrected and real-note validated on selective branch | Page additions, removals, replacements, and ordering need runtime validation | Synthetic fallback extraction + multi-image export/embed tests + real two-page scan | Retest the final candidate before release |
...
| Scanned document PDF and markup variants | Export the latest scan as PDF/images with useful links or embeds | Unknown | Scan add/remove/markup should trigger export | Multi-page image-backed scan covered; PDF/markup variants not covered | Audit PDF-backed and marked-up scan representations |
| Drawings | Export drawing image/file with a byte-correct filename extension and useful Markdown reference | Improved on selective branch | Drawing edits should trigger export | Byte-signature fallback extension test + real handwritten attachment | Retest the final candidate and audit edited drawings |
...
| Locked/password-protected notes | Export title or fallback ID plus placeholder only, report account/folder location, and avoid treating blank body as normal content | Partial | Lock/unlock state should affect fingerprint without blocking sync | Reporting + unreadable fallback + HTML/Markdown placeholder tests | Add end-to-end locked-note export fixture |
